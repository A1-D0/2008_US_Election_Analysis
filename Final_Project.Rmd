...
empirical_logit_plot <- function(data, predictor, response) {
  data %>%
    mutate(BinnedPredictor = cut(!!sym(predictor), breaks = 10)) %>%  # Bin predictor into 10 intervals
    group_by(BinnedPredictor) %>%
    summarize(
      MeanPredictor = mean(!!sym(predictor), na.rm = TRUE),  # Mean of predictor within each bin
      SuccessCount = sum(!!sym(response)),  # Count of successes (ObamaWin = 1)
      TotalCount = n(),  # Total count within each bin
      ProportionSuccess = SuccessCount / TotalCount  # Proportion of successes
    ) %>%
    mutate(
      OddsSuccess = ProportionSuccess / (1 - ProportionSuccess),  # Calculate odds
      EmpiricalLogit = log(OddsSuccess)  # Calculate logit (log-odds)
    ) %>%
    filter(!is.infinite(EmpiricalLogit)) %>%  # Remove rows with infinite logits
    ggplot(aes(x = MeanPredictor, y = EmpiricalLogit)) +
    geom_point() +
...
empirical_logit_plot(Election08, "Income", "ObamaWin")
empirical_logit_plot(Election08, "HS", "ObamaWin")
empirical_logit_plot(Election08, "BA", "ObamaWin")
empirical_logit_plot(Election08, "Dem.Rep", "ObamaWin")
```
