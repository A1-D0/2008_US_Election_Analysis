...
empirical_logit_tidy <- function(data, predictor, response) {
  ...
  emp_log <- data %>%
    mutate(BinnedPredictor = cut(!!sym(predictor), breaks = 10)) %>%
    group_by(BinnedPredictor) %>%
    summarize(
      MeanPredictor = mean(!!sym(predictor), na.rm = TRUE),
      SuccessCount = sum(!!sym(response)),
      TotalCount = n(),
      ProportionSuccess = SuccessCount / TotalCount
    ) %>%
    mutate(
      OddsSuccess = ProportionSuccess / (1 - ProportionSuccess),
      EmpiricalLogit = log(OddsSuccess)
    ) %>%
    filter(!is.infinite(EmpiricalLogit))
