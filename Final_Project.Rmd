...
# Check Conditions
The conditions for logistic regressions are linearity, randomness, and independence. The linearity condition means that a linear relationship between the log(odds) and the predictor should exist. Also, we assume that the process of data collection is random and each observation is independent.

```{r}
empirical_logit_plot_refined <- function(data, predictor, response) {
  # Bin the predictor variable into equal-width bins
  data$BinnedPredictor <- cut(data[[predictor]], breaks = 10, include.lowest = TRUE)

  summary_data <- data %>%
    group_by(BinnedPredictor) %>%
    summarise(
      MeanPredictor = mean(!!sym(predictor), na.rm = TRUE),
      SuccessCount = sum(!!sym(response), na.rm = TRUE),
      TotalCount = n(),
      .groups = "drop"
    ) %>%
    mutate(
      ProportionSuccess = SuccessCount / TotalCount,
      OddsSuccess = ifelse(
        ProportionSuccess > 0 & ProportionSuccess < 1, 
        ProportionSuccess / (1 - ProportionSuccess), 
        NA
      ),
      EmpiricalLogit = log(OddsSuccess)
    ) %>%
    filter(!is.na(EmpiricalLogit)) # Remove rows with NA logit values
...
empirical_logit_plot_refined(Election08, "Income", "ObamaWin")
empirical_logit_plot_refined(Election08, "HS", "ObamaWin")
empirical_logit_plot_refined(Election08, "BA", "ObamaWin")
empirical_logit_plot_refined(Election08, "Dem.Rep", "ObamaWin")
```
